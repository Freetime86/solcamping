...
def main(dataset):
    ...
    while True:
        ...
        date_str_begin = datetime.now().strftime("%Y-%m-%d") + ' 21:18:00'
        date_str_end = datetime.now().strftime("%Y-%m-%d") + ' 21:18:15'
                ...
                for date in sel_date_list:
                    ...
                    if sel_month == month:
                        try:
                            ...
                            retry_moudule(site, target_date, room, fix_room_num, thread_name, continue_work, trying, user_phone1, user_phone2, user_phone3)

        time.sleep(delay)


def retry_moudule(site, target_date, room, fix_room_num, thread_name, continue_work, trying, user_phone1, user_phone2, user_phone3):
    print(thread_name + ' 재실행중~~~')
    url = 'https://camping.gtdc.or.kr/DZ_reservation/procedure/execCamping_tracking.json'  # 솔향기 커넥션 정보 GET
    data = {
        'actFile': 'tracking',
        'actMode': 'Areain',
