...
if user_type == 0:
    ...
    user_name = '박준호'   #든바다 104, 107, 114, 118
    rpwd = '!asdf1357'
    rid = 'trap0959'
    rphone = '01050256509'
elif user_type == 12:
...
def main(DATASET):
    ...
    while True:
        try:
            ...
            error(DATASET)
            # if 'loginForm' in DATASET['RESULT']:
            DATASET = relogin(DATASET)
            continue
...
def reservationList_filter(DATASET):
    ...
    if len(DATASET['CANCEL_ROOMS']) > 0:
        message(DATASET, '선택 대상 -> ' + str(DATASET['ROOM_SELT']) + ' /// 취소 예약가능 대상 -> ' + str(DATASET['CANCELING_ROOMS']) + ' 실시간 매칭 대상을 확인 합니다..')
    # SPOT 모드
                    ...
                    if str(name) in DATASET['ROOM_SELT'] or len(DATASET['ROOM_SELT']) == 0:
                        ...
                        if DATASET['RESULT']['status_code'] == 200 and 'rsltMsg' in DATASET['RESULT']:
                            if DATASET['RESULT']['rsltMsg'] == '선택하신 시설이 선점되었습니다.':
                                ...
                                message(DATASET, '취소 건 강제 시도 중 ' + str(room['fcltyNm']))
                                DATASET = get_facility(DATASET)
                                break
                        else:
                            if NO_AVAILABLE:
...
def temporary_hold(DATASET):
    ...
    OPEN_TIME = datetime.now().strftime("%Y-%m-%d") + ' 11:00:00'
    ...
    CURRENT_TIMER = datetime.now()
    if (START_TIMER <= CURRENT_TIMER <= END_TIMER) or (CURRENT_TIMER >= OPEN_TIMER or DATASET['MODE_ALWS']):
